...
- 十三：展示上限统一决策与首次渲染竞态补充（2026-08-09）
...
## 十三、展示上限统一与渲染竞态补充战役（2026-08-09，CMS v2.3.4 / CME v2.4.6）

> 本次战役把「记忆/信息条目展示」与「首次渲染偶发报错」两条线收尾。核心决策来自用户对手机端插件定位的明确拍板，属于产品决策与平台约束的典型案例。

### 13.1 展示上限统一决策：手机端插件不负责大规模展示

- **背景**：CMS/CME 的记忆与六类生活数据随使用增长，原方案设计过「分页 + 批量删除 + 后端全量搜索」（双端 DEVELOPMENT_PLAN 第 13 章）。
- **用户决策（2026-08-09）**：手机端插件受平台性能限制，**不应负责大规模展示记忆与增删查改**。不做分页，统一所有条目最多显示**时间最近的 100 条、按从新到旧排序**，CMS/CME 同样处理。
- **边界**：数据不删除（仅展示截断），注入/分析链路不受影响；「分页/懒渲染」等待 Operit 平台支持差异更新后再评估。
- **落地**：
  - **CMS v2.3.4**：`load_saved_data` 六类每类 `limitLatest100`（按 timestamp 新→旧截断 100）；`load_memories` 全量浏览（query='*'）改按时间新→旧排序，**关键词/向量检索保持相关度排序**；新增 `_itemTs` 兼容秒/毫秒/ISO/startDate 时间格式。
  - **CME v2.4.6**：worker `list_memories` 默认 200→100、`load_life_data` 每类 5000→100；前端 character.js limit 200→100。
  - **消息页对话列表**（双端 4 文件）也统一 100：screen.js + messages.js 各首屏/加载更多/hasMore 的 limit 200→100。
- **教训**：① 手机端插件在「展示规模」上优先服从平台性能与产品定位，不做服务端才合理的复杂抽象；② 排序语义要区分「全量浏览（时间序）」与「检索（相关度序）」两个场景。

### 13.2 首次渲染偶发 `not a function`：与 12.1 同源的平台首次渲染竞态

- **现象**（用户实机）：CMS 时间线页/角色页**首次进入偶发** `Script error: not a function`（`_operit_render_compose_dsl`，module=theme.js from=character.js）；**切换 tab 即恢复，后续多次测试不复现**。
- **排查结论**：
  - 本地 4 分支复现（非空记忆/未识别角色/加载态/null persona）全部 RENDER OK；
  - 安装包完整、双端组件集合一致（CMS 无 CME 独有的 SelectionContainer）、UI 注册 id 无冲突；
  - 报错时 CMS enabled=true（用户已切到 CMS）；
  - 前端日志无留痕（dbgUi 探针 v1.8.4 已关闭，符合设计）。
- **定性**：首次进入偶发 + 切换恢复 + 多次不复现 = **与 12.1（快速切换界面偶发加载失效）同源的平台首次渲染 bundle 竞态**，非稳定代码缺陷。
- **缓解**：CMS v2.3.4 对 `screen.js` + `character.js` 的 `theme.c` 调用加**类型防御**（theme 模块异常时降级内置兜底色）。
- **教训**：① 排查「偶发首次报错」先做本地分支复现 + 安装包完整 + 组件集合 + id 冲突四查，再下「插件代码缺陷」结论；② 平台首次渲染竞态类的偶发错误，插件侧只能做防御性兜底，根治需平台日志定位；③ 用户补充的「切换 tab 恢复、多次不复现」是关键定性信息，务必完整记录。
