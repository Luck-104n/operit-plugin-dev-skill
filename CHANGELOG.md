...
- `CASE_STUDIES_CMS_CME.md` 新增第十三节：展示上限统一决策（CMS v2.3.4 / CME v2.4.6，最近 100 条 + 新→旧）与首次渲染偶发 `not a function` 竞态定性。
- `DEBUG_PLAYBOOK.md` 新增「首次进入偶发报错（切走/重进即恢复）」排查条目：本地分支复现、安装包完整性、组件集合一致性、启用状态确认、关键定性信息、插件侧防御兜底。

### 改动原因

- CMS/CME 2026-08-09 完成「展示上限统一为最近 100 条」与「首次渲染偶发报错定性为平台竞态」两条经验，需要沉淀为可复用案例与排障方法。

### 用户影响

- 案例文档新增手机端插件「展示规模服从平台性能定位」的产品决策记录。
- 排障手册新增「偶发首次报错」的代码层四查流程，避免把平台竞态误判为插件稳定缺陷。
